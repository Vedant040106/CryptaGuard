...
const ModuleSystem = {
    ...
    askAI: function () {
        ...
        terminal.innerHTML += `<div class="log-entry" style="color:#fff;">> USER: ${q}</div>`;
        inputField.value = '';
        terminal.innerHTML += `<div class="log-entry" id="ai-loading" style="color:var(--gold);">... Establishing Uplink ...</div>`;
        ...
        fetch('/api/ask_ai', {
            ...
            .then(data => {
                ...
                let responseText = data.response || "Connection Interrupted.";
                // Simple markdown parsing for better display (bolding)
                responseText = responseText.replace(/\*\*(.*?)\*\*/g, '<strong style="color:var(--cyan);">$1</strong>');

                terminal.innerHTML += `<div class="log-entry" style="color:#ddd; line-height:1.5;">> AI: ${responseText}</div>`;
                terminal.innerHTML += `<div class="log-entry" style="color:#333;">------------------------------------------------</div>`;
                terminal.scrollTop = terminal.scrollHeight;
            })
            .catch(err => {
                const loader = document.getElementById('ai-loading');
                if (loader) loader.remove();
                terminal.innerHTML += `<div class="log-entry" style="color:var(--danger);">> SYSTEM ERROR: Network Failure.</div>`;
            });
    },
    ...
    clearLog: function () {
        const term = document.getElementById('terminal');
        if (term) {
            // Keep the clear button if it's inside, or just clear content
            // The clear button is absolutely positioned, so innerHTML='' might wipe it if it's not careful.
            // My implementation in app.js puts the button INSIDE .console-output.
            // So I should rebuild it, or just remove all .log-entry elements.
            const logs = term.querySelectorAll('.log-entry');
